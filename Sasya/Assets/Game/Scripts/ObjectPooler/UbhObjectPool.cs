...
public sealed class UbhObjectPool : UbhSingletonMonoBehavior<UbhObjectPool>
{
    ...
    {
        public List<Bullet> m_bulletList = new List<Bullet>(1024);
    ...
    public void CreatePool(GameObject goPrefab, int createNum)
    {
        ...
        {
            Bullet bullet = GetBullet(goPrefab, UbhUtil.VECTOR3_ZERO, true);
            if (bullet == null)
            {
                break;
            }
    ...
    private void RemovePoolParam(PoolingParam poolParam)
    {
            ...
            poolParam.m_bulletList.Count > 0)
        {
            ...
            {
                Bullet bullet = poolParam.m_bulletList[i];
    ...
    /// </summary>
    public Bullet GetBullet(GameObject goPrefab, Vector3 position, bool forceInstantiate = false)
    {
        if (goPrefab == null)
        {
            return null;
    ...
    /// </summary>
    public void ReleaseBullet(Bullet bullet, bool destroy = false)
    {
        if (bullet == null || bullet.gameObject == null)
        {
            return;
    ...
    /// </summary>
    public List<Bullet> GetActiveBulletsList(GameObject goPrefab)
    {
        int key = goPrefab.GetInstanceID();
        List<Bullet> findList = null;
